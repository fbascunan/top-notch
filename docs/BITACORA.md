...
### 2026-04-14 — agent — M23
**Did:** built Human Actions Dashboard. Migration `00008_human_actions.sql` (table with status enum, indexes, completed_at trigger, 4 RLS policies for org members). Parser utility (`human-actions-parser.ts`) extracts structured actions from HUMAN-ACTIONS.md markdown tables. Data layer (`human-actions-data.ts`) with Supabase + seed fallback. `HumanActionsTable.astro` component (grouped by milestone, blocker badges in red, post-deploy in blue, toggle-to-complete buttons for members). Human Actions section on `/projects/[slug]` (ES+EN, member-only). Global `/human-actions` page (ES+EN) with project and blocker-only filters. API routes: `PATCH /api/human-actions/[id]` (status toggle), `POST /api/human-actions/sync` (upsert from markdown). Navbar link for members. 30+ i18n keys (ES+EN). Build: 0 errors.
**Blocked:** Human needs to run `supabase db push` to apply migration 00008.
**Next:** M24 (Routine Setup & Trigger API) or human completes M14 external services.

### 2026-04-14 — agent + human — M19–M22 setup / M24–M27 planning
**Did:** applied Supabase migration `00007_run_history.sql` via `supabase db push`. Set GitHub Secrets (`SUPABASE_URL` real, `SUPABASE_SERVICE_ROLE_KEY` and `ANTHROPIC_API_KEY` as placeholders). Discovered critical flaw: M20–M22 require `ANTHROPIC_API_KEY` (pay-per-token) but user has Claude Max subscription. Researched Claude Code Routines as replacement — runs on Anthropic cloud using subscription, supports API triggers and cron schedules. Designed and spec-reviewed migration plan. Wrote milestones M24–M27 in MILESTONES.md: M24 (routine setup + trigger API), M25 (webhook listener), M26 (scheduled routine / Ralph Loop), M27 (cleanup).
**Blocked:** M24 implementation requires reading Claude Code Routines docs (new feature, API surface may change). Human needs to update fake GitHub Secrets with real values (SUPABASE_SERVICE_ROLE_KEY from Supabase dashboard, ANTHROPIC_API_KEY from console.anthropic.com) — though ANTHROPIC_API_KEY may become unnecessary after M27.
**Next:** implement M24 — create routine, rewire API trigger, add schema migration
**Decision:** M20–M22 GitHub Actions approach abandoned in favor of Claude Code Routines (subscription-based, no API billing). Routine codes + pushes, webhook tracks results in Supabase — separation of concerns. Correlation via UUID in commit messages `[run:<id>]`. Priority for scheduled runs comes from MILESTONES.md tracker (not workspace MANIFEST.md which is outside the repo).

### 2026-04-14 — agent — M24
**Did:** rewired milestone runner from GitHub Actions `workflow_dispatch` to Claude Code Routines `/fire` API. Read routines docs via Context7 + web fetch. Migration `00009_run_history_routines.sql` adds `correlation_id` (TEXT, unique where not null) and `trigger_source` (TEXT, default `'manual'`) to `run_history`. Updated `database.types.ts` and `RunHistoryEntry`. Rewrote `POST /api/run-milestone`: generates UUID `correlation_id`, POSTs to `https://api.anthropic.com/v1/claude_code/routines/{trigger_id}/fire` with bearer token + beta header, stores session URL. Added stale-run cleanup (auto-fail queued/running >2h). Error handling: 429 rate limit, 401/403 auth, timeout (30s), generic failures. Documented routine commit format for M25. Added 6 human-action items in HUMAN-ACTIONS.md for routine creation + token setup. Build: 0 errors, 0 warnings, 87 files checked.
...
### 2026-04-15 — agent — M27
**Did:** cleaned up old GitHub Actions runner code after M24–M26 migration to Claude Code Routines. Deleted `.github/workflows/run-milestone.yml` (old workflow) and `.github/scripts/supabase-runner.mjs` (entirely dead — webhook uses `supabase-helpers.mjs` directly). Removed dead `updateTasksFromOutput()` from `supabase-helpers.mjs`. Cleaned stale comments referencing `workflow_dispatch`, `ANTHROPIC_API_KEY`, and `GITHUB_TOKEN` from source code. Updated `HUMAN-ACTIONS.md` — marked M20/M21/M22 items as obsolete, added M27 section for `ANTHROPIC_API_KEY` secret removal and routine verification. Updated seed data (M20/M21/M22 human actions marked done). Updated MILESTONES.md tracker (M27 → Done). Build passes with 0 errors.
...
### 2026-04-17 13:03 — runner — (none) [run:scheduled-2026-04-17T13:03:00.000Z]
**Did:** scanned Tracker table in MILESTONES.md — no Planned milestones remaining. All M1–M27 are Done.
**Blocked:** No Planned milestones remaining. Pending human actions: Formspree, Umami, DNS, Google Search Console, responsive QA, manual+scheduled trigger verification (see HUMAN-ACTIONS.md).
**Next:** define new milestones in MILESTONES.md to continue work, or complete remaining post-deploy human actions

### 2026-04-19 13:03 — runner — (none) [run:scheduled-2026-04-19T13:03:00.000Z]
**Did:** scanned Tracker table in MILESTONES.md — no Planned milestones remaining. All M1–M27 are Done.
**Blocked:** No Planned milestones remaining. Pending human actions: Formspree, Umami, DNS, Google Search Console, responsive QA, manual+scheduled trigger verification (see HUMAN-ACTIONS.md).
**Next:** define new milestones in MILESTONES.md to continue work, or complete remaining post-deploy human actions

### 2026-04-20 13:03 — runner — (none) [run:scheduled-2026-04-20T13:03:00.000Z]
**Did:** scanned Tracker table in MILESTONES.md — no Planned milestones remaining. All M1–M27 are Done.
**Blocked:** No Planned milestones remaining. Pending human actions: Formspree, Umami, DNS, Google Search Console, responsive QA, manual+scheduled trigger verification (see HUMAN-ACTIONS.md).
**Next:** define new milestones in MILESTONES.md to continue work, or complete remaining post-deploy human actions

### 2026-04-21 13:03 — runner — (none) [run:scheduled-2026-04-21T13:03:00.000Z]
**Did:** scanned Tracker table in MILESTONES.md — no Planned milestones remaining. All M1–M27 are Done.
**Blocked:** No Planned milestones remaining. Pending human actions: Formspree, Umami, DNS, Google Search Console, responsive QA, manual+scheduled trigger verification (see HUMAN-ACTIONS.md).
**Next:** define new milestones in MILESTONES.md to continue work, or complete remaining post-deploy human actions
...
### 2026-04-23 13:03 — runner — (none) [run:scheduled-2026-04-23T13:03:00.000Z]
**Did:** scanned Tracker table in MILESTONES.md — no Planned milestones remaining. All M1–M27 are Done.
**Blocked:** No Planned milestones remaining. Pending human actions: Formspree, Umami, DNS, Google Search Console, responsive QA, manual+scheduled trigger verification (see HUMAN-ACTIONS.md).
**Next:** define new milestones in MILESTONES.md to continue work, or complete remaining post-deploy human actions

### 2026-04-24 13:03 — runner — (none) [run:scheduled-2026-04-24T13:03:00.000Z]
**Did:** scanned Tracker table in MILESTONES.md — no Planned milestones remaining. All M1–M27 are Done.
**Blocked:** No Planned milestones remaining. Pending human actions: Formspree, Umami, DNS, Google Search Console, responsive QA, manual+scheduled trigger verification (see HUMAN-ACTIONS.md).
**Next:** define new milestones in MILESTONES.md to continue work, or complete remaining post-deploy human actions
